...
class App {
private:
  static constexpr const char* APP_NAME = "10_mesh";
...
App::App()
{
  ...
  if (!ac_device_support_mesh_shaders(m_device))
  {
    return;
  }
  ...
  {
    ac_shader_info info = {};
    info.stage = ac_shader_stage_mesh;
    info.code = main_mesh[0];
    info.name = AC_DEBUG_NAME(App::APP_NAME);

    RIF(ac_create_shader(m_device, &info, &m_mesh_shader));
  }
...
App::~App()
{
  if (m_device)
  {
    ...
    ac_destroy_shader(m_mesh_shader);
...
App::create_window_dependents()
{
...

  {
    ...
    info.type = ac_pipeline_type_mesh;
    info.name = AC_DEBUG_NAME(App::APP_NAME);
    info.mesh.mesh_shader = m_mesh_shader;
    info.mesh.pixel_shader = m_fragment_shader;
    info.mesh.dsl = m_dsl;
    info.mesh.samples = 1;
    info.mesh.color_attachment_count = 1;
    info.mesh.color_attachment_formats[0] = ac_image_get_format(image);
...
App::stage_cmd(ac_rg_stage* stage, void* ud)
{
  ...
  ac_cmd_draw_mesh_tasks(cmd, 1, 1, 1);
